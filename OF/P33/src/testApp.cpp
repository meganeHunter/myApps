...
void testApp::update()
{
	...
	for(k=0;k<mRect.size();k++)
	{
		...
		for(j=0;j<mRect.size();j++)
		{
			...
			{
				/*
				if(-1.0 * sRectW < distX && distX < 0.0)
				{
					mRect[k]->nearX = true;
					if(mRect[k]->nearX && mRect[k]->rectMov)
					{
			...
			/*
			if(sPos.y < fh - fPos.y && mRect[k]->rectMov)
			{
				sPos += sG;
				dist = ofDist(sPos.x,sPos.y,vPos.x,vPos.y);

				if(dist < mRect[j]->rectW * 2.0 && j != k)
				{
			...
			else if(fh - fPos.y < sPos.y && sPos.y < fh)
			{
				psPos = sPos;
				distX = (unsigned)(sPos.x - vPos.x);
				
				mRect[k]->rectMov = false;
				sPos = psPos;
				
				if(!mRect[j]->rectMov && distX < mRect[j]->rectW * 2.0)
				{
					if(sPos.y > vPos.y - mRect[k]->rectH && j != k)
					{
						sPos.y = vPos.y - mRect[k]->rectH;
					}
				}
